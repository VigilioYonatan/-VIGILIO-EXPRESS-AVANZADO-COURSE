		...
		<div
			...
			class={`${
				isActive || dropdownOpen
					? "dark:bg-admin-terciary bg-paper-light [>&svg]:text-terciary-light "
					: "text-terciary-light dark:hover:bg-admin-terciary hover:bg-paper-light"
		...
		>
			<div class="flex justify-between items-center w-full py-1">
				<Link {...rest}>
					<button
						type="button"
						class="flex gap-2 items-center w-full dark:hover:text-white dark:text-terciary-dark text-secondary-dark"
					>
						{Icon}
	...
	return (
		<Link key={link.to} to={link.to}>
			<button
				...
				class={`${
					isActive ? "" : ""
				} font-semibold sm:pl-2 w-full p-1.5 dark:hover:text-white dark:text-terciary-dark text-secondary-dark text-start`}
			>
				{link.title}
			</button>
		</Link>
